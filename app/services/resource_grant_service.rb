class ResourceGrantService
  def self.grant!(user:, resource:, granted_by:, access_level: "read", expires_in: 30.days, metadata: {})
    ...
    AuditEvent.log!(
      ...
      auditable: resource,
      metadata: metadata.merge(grantee_user_id: user.id, access_level: access_level)
    )
